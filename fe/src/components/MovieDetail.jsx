...
import { Container, Row, Col, Carousel, Form, Button, Spinner, Alert  } from "react-bootstrap";
...
const MovieDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchMovieData = async () => {
      const movieUrl = `${URL}/movies/${id}`; // URL dari backend lokal
      ...
      try {
        ...
        setTrailers(movieData.movie.MovieVideos);
        setActors(movieData.movie.Actors);
        setRecommendedMovies(movieData.recommendations);
        setDirector(movieData.movie.Director?.name || "Unknown");
  ...
  const getCountryName = (countryId) => {
    ...
    return countryItem ? countryItem.name : "Unknown"; // Jika tidak ditemukan, tampilkan 'Unknown'
  };
  ...
  return (
    <div>
      ...
      <Container className="mt-5">
        ...
        {trailers.length > 0 && trailers.some((trailer) => trailer.url) ? (
          <div className="carousel-wrapper">
            <Carousel className="trailer-carousel">
              {trailers.map(
                (trailer) =>
                  trailer.url ? ( // Check if URL exists
          ...
          <p className="text-white">No Trailers Available</p> // Menampilkan pesan jika tidak ada trailer
        )}
      </Container>
